...
const App = () => {
  ...
  }, [token, activeTab, filters.category, filters.type]);
    ...
    <div className="container" style={{ padding: '7rem 2rem 5rem' }}>
      ...
      <nav className="glass" style={{ position: 'fixed', top: '1.5rem', left: '2rem', right: '2rem', padding: '1rem 2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 1000 }}>
        <div style={{ display: 'flex', gap: '2.5rem', alignItems: 'center' }}>
          <h2 className="glass-title" style={{ fontSize: '1.75rem' }}>FintechAI</h2>
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button onClick={() => setActiveTab('dashboard')} className={activeTab === 'dashboard' ? 'btn-primary' : 'btn-icon'} style={{ padding: '0.5rem 1rem' }}>
              <LayoutDashboard size={18} /> Dashboard
            </button>
            {user.role === 'Admin' && (
              <button onClick={() => setActiveTab('users')} className={activeTab === 'users' ? 'btn-primary' : 'btn-icon'} style={{ padding: '0.5rem 1rem' }}>
                <Users size={18} /> User Control
              </button>
            )}
            <button onClick={() => setActiveTab('documentation')} className={activeTab === 'documentation' ? 'btn-primary' : 'btn-icon'} style={{ padding: '0.5rem 1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          ...
          <button className="btn-icon" onClick={handleLogout}><LogOut size={20} /></button>
        </div>
      </nav>
